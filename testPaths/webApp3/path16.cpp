// Flow 16: Duplicate Item Flow
// Path: Login → Add ItemX → Add same ItemX again → Cart shows quantity=2 → Order
// Expected: SAT (quantity should increment properly)
...
static Program buildClientProgram()
{
    ...
    // add_to_cart(item_X) - First time
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("item_X"));
        stmts.push_back(make_unique<FuncCallStmt>(make_unique<FuncCall>("add_to_cart", move(args))));
    }

    // add_to_cart(item_X) - Second time
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("item_X"));
        stmts.push_back(make_unique<FuncCallStmt>(make_unique<FuncCall>("add_to_cart", move(args))));
...
static Spec buildSpec()
{
    ...
    {
        vector<unique_ptr<Expr>> pre;
        pre.push_back(make_unique<Var>("T"));
        pre.push_back(make_unique<Var>("token"));
    ...
    // order
    {
        ...
        Response resp(HTTPResponseCode::CREATED_201, make_unique<Var>("\"Order successful\""));
        auto apicall = make_unique<APIcall>(std::move(callFn), std::move(resp));
