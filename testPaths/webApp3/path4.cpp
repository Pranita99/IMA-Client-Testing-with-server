//Path 4: input(username), input(password) → login success(username, password) → get-
//menu(canteen 3) → add to cart(item 4), add to cart(item 5) → order(cart)
...
static Program buildClientProgram()
{
    ...
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("canteen_3"));
        stmts.push_back(make_unique<FuncCallStmt>(
    ...
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("item_4"));
        stmts.push_back(make_unique<FuncCallStmt>(
    ...
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("item_5"));
        stmts.push_back(make_unique<FuncCallStmt>(
            make_unique<FuncCall>("add_to_cart", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- getmenu ---
    {
        ...
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("canteen_3"));
    ...
    // --- add_to_cart (item_4 & item_5) ---
    for (string item : {"item_4", "item_5"}) {
        ...
        postArgs.push_back(make_unique<Var>(item));
