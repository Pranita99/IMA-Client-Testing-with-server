// Flow: Login → Order → Logout
// Path: Login → Place order → Logout
// Expected: SAT (should successfully place order and logout)
...
static Spec buildSpec()
{
    ...
    // Initialize cart with some items for successful order
    vector<pair<unique_ptr<Var>, unique_ptr<Expr>>> cart_pairs;
    cart_pairs.push_back(make_pair(make_unique<Var>("\"pizza\""), make_unique<Var>("2")));
    cart_pairs.push_back(make_pair(make_unique<Var>("\"burger\""), make_unique<Var>("1")));
    inits.push_back(make_unique<Init>("cart", make_unique<Map>(move(cart_pairs))));
