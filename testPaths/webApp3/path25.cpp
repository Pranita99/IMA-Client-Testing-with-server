// Flow: Login → Add to Cart → Remove Item → Logout → Login → Order
// Path: Login → Add items → Remove item → Logout → Login again → Place order
...
static Program buildClientProgram()
{
    ...
    // First login_success(username, password)
    {
    ...
    // Second login_success(username, password) - Login again
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("username"));
        args.push_back(make_unique<Var>("password"));
    ...
    {
        vector<unique_ptr<Expr>> args;
        stmts.push_back(make_unique<FuncCallStmt>(make_unique<FuncCall>("place_order", move(args))));
    }
...
static Spec buildSpec()
{
    ...
    // place_order
    {
        ...
        gtArgs.push_back(make_unique<Var>("0"));
        conj.push_back(make_unique<FuncCall>("greater_than", move(gtArgs)));

        // User must be logged in (token exists)
        vector<unique_ptr<Expr>> inArgs;
        inArgs.push_back(make_unique<Var>("token"));
        { vector<unique_ptr<Expr>> h; h.push_back(make_unique<Var>("T"));
          inArgs.push_back(make_unique<FuncCall>("dom", move(h))); }
    ...
    globals.push_back(make_unique<Decl>("orders", make_unique<MapType>(make_unique<TypeConst>("string"), make_unique<TypeConst>("string"))));

    vector<unique_ptr<Init>> inits;
    // Initialize with a user already in the system (since we skip signup)
    vector<pair<unique_ptr<Var>, unique_ptr<Expr>>> user_pairs;
    user_pairs.push_back(make_pair(make_unique<Var>("username"), make_unique<Var>("password")));
    inits.push_back(make_unique<Init>("U", make_unique<Map>(move(user_pairs))));
