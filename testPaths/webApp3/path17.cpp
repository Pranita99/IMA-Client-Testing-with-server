// Flow: Login → Get Menu
// Path: Login → Get menu items
// Expected: SAT (should successfully return menu after login)
...
static Spec buildSpec()
{
    ...
    // get_menu (successful case)
    {
        ...
        // Return menu items (could be represented as a map or list)
    ...
    // Initialize menu_items with some sample data
    vector<pair<unique_ptr<Var>, unique_ptr<Expr>>> menu_pairs;
    menu_pairs.push_back(make_pair(make_unique<Var>("\"item1\""), make_unique<Var>("\"Pizza\"")));
    menu_pairs.push_back(make_pair(make_unique<Var>("\"item2\""), make_unique<Var>("\"Burger\"")));
    menu_pairs.push_back(make_pair(make_unique<Var>("\"item3\""), make_unique<Var>("\"Salad\"")));
