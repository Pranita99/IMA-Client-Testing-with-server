// Flow 7:
// signup → login → addItemA → addItemB → removeItemA → order
// Expected: SAT (valid path where cart has item_B before ordering)
...
static Program buildClientProgram()
{
    ...
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("item_A"));
    ...
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("item_B"));
        stmts.push_back(make_unique<FuncCallStmt>(make_unique<FuncCall>("add_to_cart", move(args))));
    }
    ...
    // remove_from_cart(item_A)
    {
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>("item_A"));
        stmts.push_back(make_unique<FuncCallStmt>(make_unique<FuncCall>("remove_from_cart", move(args))));
    }
