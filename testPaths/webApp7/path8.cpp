...
//        → add_to_cart → place_order → get_orders → get_order_details
// ─────────────────────────────────────────────────────────────
...
static Program buildClientProgram()
{
    ...
    // get_orders();
    {
        vector<unique_ptr<Expr>> a;
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("get_orders", move(a))));
    }
    ...
    // get_order_details(order_id);
    {
        ...
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("get_order_details", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- Get Orders API block ---
    {
        ...
        /* call */
        vector<unique_ptr<Expr>> callArgs;
        auto callFn = make_unique<FuncCall>("get_orders", move(callArgs));

        /* post: returns list of user's orders */
        auto post = make_unique<FuncCall>("true", vector<unique_ptr<Expr>>{});
    ...
    // --- Get Order Details API block ---
    {
        /* pre: token ∈ dom(T) && order_id ∈ dom(O) && O[order_id].user = T[token] */
        vector<unique_ptr<Expr>> inDomT;
        inDomT.push_back(make_unique<Var>("T"));
        inDomT.push_back(make_unique<Var>("token"));
        ...
        vector<unique_ptr<Expr>> orderOwnership;
        {
            vector<unique_ptr<Expr>> orderUser;
            {
                vector<unique_ptr<Expr>> orderIdx;
                orderIdx.push_back(make_unique<Var>("O"));
                orderIdx.push_back(make_unique<Var>("order_id"));
                orderUser.push_back(make_unique<FuncCall>("mapped_value", move(orderIdx)));
            }
            orderUser.push_back(make_unique<Var>("user"));
            
            vector<unique_ptr<Expr>> tokenUser;
            tokenUser.push_back(make_unique<Var>("T"));
            tokenUser.push_back(make_unique<Var>("token"));
            
            orderOwnership.push_back(make_unique<FuncCall>("field_access", move(orderUser)));
            orderOwnership.push_back(make_unique<FuncCall>("mapped_value", move(tokenUser)));
        }
        
        vector<unique_ptr<Expr>> land;
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomT)));
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomO)));
        land.push_back(make_unique<FuncCall>("equals", move(orderOwnership)));
        auto pre = make_unique<FuncCall>("and_operator", move(land));

        /* call */
        vector<unique_ptr<Expr>> callArgs;
        callArgs.push_back(make_unique<Var>("order_id"));
        auto callFn = make_unique<FuncCall>("get_order_details", move(callArgs));

        /* post: returns order details for order_id */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> idx;
        ...
        postArgs.push_back(make_unique<FuncCall>("order_details", vector<unique_ptr<Expr>>{}));
        auto post = make_unique<FuncCall>("equals", move(postArgs));
    ...
    globals.push_back(make_unique<Decl>(
        "C", make_unique<MapType>(
                 ...
                 make_unique<MapType>(
                     make_unique<TypeConst>("string"),
                     make_unique<TypeConst>("string")))));
    // Orders map (order_id -> order details)
    globals.push_back(make_unique<Decl>(
