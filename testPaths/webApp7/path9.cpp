...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for signup → login → get_restaurants → get_restaurant_menu → add_to_cart → place_order → add_review
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // add_review(order_id, rating, comment);
    {
        vector<unique_ptr<Expr>> a;
...
// ─────────────────────────────────────────────────────────────
//  Build the API *Spec* AST with restaurant and review functionality
// ─────────────────────────────────────────────────────────────
static Spec buildSpec()
{
    ...
    // ═══ GET RESTAURANTS API BLOCK ═══
    {
        ...
        /* post: returns list of restaurants */
    ...
    // ═══ ADD TO CART API BLOCK ═══
    {
        ...
        /* post: C[user_id][item_id] = quantity */
        vector<unique_ptr<Expr>> postArgs;
        {
    ...
    // ═══ PLACE ORDER API BLOCK ═══
    {
        ...
        vector<unique_ptr<Expr>> land;
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomT)));
        land.push_back(make_unique<FuncCall>("true", move(cartNotEmpty)));
        auto pre = make_unique<FuncCall>("and_operator", move(land));
    ...
    {
        /* pre: in_dom(token, T) && in_dom(order_id, O) && order_belongs_to_user */
        vector<unique_ptr<Expr>> inDomT;
        inDomT.push_back(make_unique<Var>("token"));
        inDomT.push_back(make_unique<Var>("T"));
        ...
        // Check if order belongs to user (simplified)
        vector<unique_ptr<Expr>> orderBelongsToUser;
        {
            vector<unique_ptr<Expr>> userIdx;
            userIdx.push_back(make_unique<Var>("T"));
            userIdx.push_back(make_unique<Var>("token"));
            ...
            vector<unique_ptr<Expr>> orderCheck;
            orderCheck.push_back(make_unique<Var>("O"));
            orderCheck.push_back(make_unique<Var>("order_id"));
            orderCheck.push_back(std::move(userId));
            orderBelongsToUser.push_back(make_unique<FuncCall>("order_belongs_to_user", move(orderCheck)));
        }
       
        vector<unique_ptr<Expr>> land;
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomT)));
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomO)));
        land.push_back(make_unique<FuncCall>("true", move(orderBelongsToUser)));
        auto pre = make_unique<FuncCall>("and_operator", move(land));
        ...
        /* post: RV[review_id] = review_details */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> reviewIdx;
            reviewIdx.push_back(make_unique<Var>("RV"));
            reviewIdx.push_back(make_unique<Var>("review_id"));
            postArgs.push_back(make_unique<FuncCall>("mapped_value", move(reviewIdx)));
            
            vector<unique_ptr<Expr>> reviewDetails;
            reviewDetails.push_back(make_unique<Var>("order_id"));
            {
                vector<unique_ptr<Expr>> userIdx;
                userIdx.push_back(make_unique<Var>("T"));
                userIdx.push_back(make_unique<Var>("token"));
                reviewDetails.push_back(make_unique<FuncCall>("mapped_value", move(userIdx)));
            }
            reviewDetails.push_back(make_unique<Var>("rating"));
            reviewDetails.push_back(make_unique<Var>("comment"));
            postArgs.push_back(make_unique<FuncCall>("review_data", move(reviewDetails)));
        }
        auto post = make_unique<FuncCall>("equals", move(postArgs));
    ...
    globals.push_back(make_unique<Decl>(
        "O", make_unique<MapType>(
                 make_unique<TypeConst>("string"),
                 make_unique<TypeConst>("string"))));
    // Reviews map: review_id -> review_details
    globals.push_back(make_unique<Decl>(
