...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for signup → login → get_restaurants → get_restaurant_menu → add_to_cart → place_order → get_orders
// ─────────────────────────────────────────────────────────────
...
static Spec buildSpec()
{
    ...
    // ═══ GET ORDERS API BLOCK ═══
    {
        ...
        /* post: returns all orders for current user */
        vector<unique_ptr<Expr>> postArgs;
        {
            ...
            // Filter orders by user_id from order details
            vector<unique_ptr<Expr>> userOrders;
            userOrders.push_back(make_unique<Var>("O"));
            userOrders.push_back(std::move(userId));
            postArgs.push_back(make_unique<FuncCall>("filter_orders_by_user", move(userOrders)));
        }
        postArgs.push_back(make_unique<FuncCall>("user_orders", vector<unique_ptr<Expr>>{}));
        auto post = make_unique<FuncCall>("equals", move(postArgs));
