...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for signup → login → get_restaurants → get_restaurant_menu → add_to_cart → remove_from_cart
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // ═══ REMOVE FROM CART ═══
    // string remove_item_id;
    decls.push_back(make_unique<Decl>("remove_item_id",
                     make_unique<TypeConst>("string")));
    // remove_item_id = input();
    {
        auto lhs = make_unique<Var>("remove_item_id");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(args))));
    ...
    // remove_from_cart(remove_item_id);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("remove_item_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
...
static Spec buildSpec()
{
    ...
    // ═══ REMOVE FROM CART API BLOCK ═══
    {
        /* pre: in_dom(token, T) && in_dom(remove_item_id, user_cart) */
        vector<unique_ptr<Expr>> inDomT;
        inDomT.push_back(make_unique<Var>("token"));
        inDomT.push_back(make_unique<Var>("T"));
       
        // Check if item exists in user's cart
        vector<unique_ptr<Expr>> userCartExists;
        {
            vector<unique_ptr<Expr>> userIdx;
            userIdx.push_back(make_unique<Var>("T"));
            userIdx.push_back(make_unique<Var>("token"));
            ...
            cartIdx.push_back(make_unique<Var>("C"));
            cartIdx.push_back(std::move(userId));
            cartIdx.push_back(make_unique<Var>("remove_item_id"));
            userCartExists.push_back(make_unique<FuncCall>("nested_in_dom", move(cartIdx)));
        }
       
        vector<unique_ptr<Expr>> land;
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomT)));
        land.push_back(make_unique<FuncCall>("true", move(userCartExists))); // Simplified check
        auto pre = make_unique<FuncCall>("and_operator", move(land));

        /* call */
        vector<unique_ptr<Expr>> callArgs;
        callArgs.push_back(make_unique<Var>("remove_item_id"));
        auto callFn = make_unique<FuncCall>("remove_from_cart", move(callArgs));

        /* post: item removed from C[user_id] (item no longer exists in user's cart) */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> userIdx;
            userIdx.push_back(make_unique<Var>("T"));
            ...
            vector<unique_ptr<Expr>> notInCart;
            notInCart.push_back(make_unique<Var>("remove_item_id"));
            {
                vector<unique_ptr<Expr>> cartDom;
                cartDom.push_back(make_unique<Var>("C"));
                cartDom.push_back(std::move(userId));
                notInCart.push_back(make_unique<FuncCall>("mapped_value", move(cartDom)));
            }
            postArgs.push_back(make_unique<FuncCall>("not_in", move(notInCart)));
        }
        auto post = make_unique<FuncCall>("true", move(postArgs)); // Simplified postcondition
