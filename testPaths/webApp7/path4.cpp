...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for signup → login → get_restaurants → get_restaurant_menu → add_to_cart → update_cart_item
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // string new_quantity;
    decls.push_back(make_unique<Decl>("new_quantity",
                     make_unique<TypeConst>("string")));
    // new_quantity = input();
    {
        auto lhs = make_unique<Var>("new_quantity");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(args))));
    ...
    // update_cart_item(update_item_id, new_quantity);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("update_item_id"));
        a.push_back(make_unique<Var>("new_quantity"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("update_cart_item", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // ═══ UPDATE CART ITEM API BLOCK ═══
    {
        /* pre: in_dom(token, T) && in_dom(update_item_id, user_cart) */
        vector<unique_ptr<Expr>> inDomT;
        inDomT.push_back(make_unique<Var>("token"));
        inDomT.push_back(make_unique<Var>("T"));
       
        // Check if item exists in user's cart (simplified)
        vector<unique_ptr<Expr>> itemInCart;
        {
            vector<unique_ptr<Expr>> userIdx;
            userIdx.push_back(make_unique<Var>("T"));
            userIdx.push_back(make_unique<Var>("token"));
            ...
            cartCheck.push_back(make_unique<Var>("update_item_id"));
            {
                vector<unique_ptr<Expr>> cartIdx;
                cartIdx.push_back(make_unique<Var>("C"));
                cartIdx.push_back(std::move(userId));
                cartCheck.push_back(make_unique<FuncCall>("mapped_value", move(cartIdx)));
            }
            itemInCart.push_back(make_unique<FuncCall>("in_dom", move(cartCheck)));
        }
       
        vector<unique_ptr<Expr>> land;
        land.push_back(make_unique<FuncCall>("in_dom", move(inDomT)));
        land.push_back(make_unique<FuncCall>("true", move(itemInCart))); // Simplified check
        auto pre = make_unique<FuncCall>("and_operator", move(land));

        /* call */
        vector<unique_ptr<Expr>> callArgs;
        callArgs.push_back(make_unique<Var>("update_item_id"));
        callArgs.push_back(make_unique<Var>("new_quantity"));
        auto callFn = make_unique<FuncCall>("update_cart_item", move(callArgs));

        /* post: C[user_id][update_item_id] = new_quantity */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> userIdx;
            userIdx.push_back(make_unique<Var>("T"));
            ...
            cartIdx.push_back(make_unique<Var>("C"));
            cartIdx.push_back(std::move(userId));
            cartIdx.push_back(make_unique<Var>("update_item_id"));
            postArgs.push_back(make_unique<FuncCall>("nested_mapped_value", move(cartIdx)));
        }
        postArgs.push_back(make_unique<Var>("new_quantity"));
        auto post = make_unique<FuncCall>("equals", move(postArgs));
