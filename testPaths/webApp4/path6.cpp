// Path 6:
// Raise_Complaint → Assign(Plumber) → Plumber_Reject → Reassign(Manager) (Valid path, should return SAT)
...
static Program buildClientProgram()
{
    ...
    // raise_complaint(complaint_description, customer_location, service_type, urgency_level);
    {
        vector<unique_ptr<Expr>> a;
    ...
    {
        auto lhs = make_unique<Var>("rejection_reason");
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(a))));
    ...
    // plumber_reject(complaint_id, plumber_id, rejection_reason);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("complaint_id"));
        a.push_back(make_unique<Var>("plumber_id"));
        a.push_back(make_unique<Var>("rejection_reason"));
        stmts.push_back(make_unique<FuncCallStmt>(
            make_unique<FuncCall>("plumber_reject", move(a))));
    }
    ...
    // reassign_complaint(complaint_id, new_plumber_id);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("complaint_id"));
        a.push_back(make_unique<Var>("new_plumber_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
            make_unique<FuncCall>("reassign_complaint", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- assign_plumber ---
    {
        ...
        // plumber is available and has required specialization
        {
            vector<unique_ptr<Expr>> args;
            args.push_back(make_unique<Var>("plumber_id"));
            args.push_back(make_unique<Var>("service_type"));
            conj.push_back(make_unique<FuncCall>("plumber_available_and_qualified", move(args)));
        }
        ...
        // ComplaintStatus[complaint_id] == STATE_ASSIGNED
        {
            vector<unique_ptr<Expr>> eq;
            eq.push_back(mapVal("ComplaintStatus", "complaint_id"));
    ...
    // --- plumber_reject ---
    {
        ...
        args.push_back(make_unique<Var>("complaint_id"));
        args.push_back(make_unique<Var>("plumber_id"));
        args.push_back(make_unique<Var>("rejection_reason"));
        auto callFn = make_unique<FuncCall>("plumber_reject", move(args));

        vector<unique_ptr<Expr>> postConj;

        // ComplaintStatus[complaint_id] == STATE_RAISED (back to raised for reassignment)
        {
            vector<unique_ptr<Expr>> eq;
            eq.push_back(mapVal("ComplaintStatus", "complaint_id"));
            eq.push_back(make_unique<Var>("STATE_RAISED"));
        ...
        // ComplaintRejections[complaint_id] contains rejection record
        {
            vector<unique_ptr<Expr>> args;
            args.push_back(make_unique<Var>("complaint_id"));
            args.push_back(make_unique<Var>("plumber_id"));
            args.push_back(make_unique<Var>("rejection_reason"));
            postConj.push_back(make_unique<FuncCall>("rejection_recorded", move(args)));
        }

        // Original assignment is cleared
        {
            vector<unique_ptr<Expr>> h;
            h.push_back(make_unique<Var>("complaint_id"));
            h.push_back(make_unique<Var>("ComplaintAssignments"));
    ...
    // --- reassign_complaint ---
    {
        ...
        // complaint is back in RAISED state (after rejection)
        {
            vector<unique_ptr<Expr>> eq;
            eq.push_back(mapVal("ComplaintStatus", "complaint_id"));
            eq.push_back(make_unique<Var>("STATE_RAISED"));
        ...
        // new plumber is different from the one who rejected
        {
            vector<unique_ptr<Expr>> args;
            args.push_back(make_unique<Var>("complaint_id"));
            args.push_back(make_unique<Var>("new_plumber_id"));
            conj.push_back(make_unique<FuncCall>("different_from_rejected_plumber", move(args)));
        }

        // new plumber is available and qualified
        {
            vector<unique_ptr<Expr>> args;
            args.push_back(make_unique<Var>("new_plumber_id"));
            args.push_back(make_unique<Var>("service_type"));
        ...
        args.push_back(make_unique<Var>("complaint_id"));
        args.push_back(make_unique<Var>("new_plumber_id"));
        auto callFn = make_unique<FuncCall>("reassign_complaint", move(args));
        ...
        // ComplaintAssignments[complaint_id] == new_plumber_id
        {
            vector<unique_ptr<Expr>> eq;
            eq.push_back(mapVal("ComplaintAssignments", "complaint_id"));
        ...
        // Reassignment count is incremented
        {
            vector<unique_ptr<Expr>> args;
            args.push_back(make_unique<Var>("complaint_id"));
            postConj.push_back(make_unique<FuncCall>("reassignment_count_incremented", move(args)));
        }
    ...
    globals.push_back(make_unique<Decl>("ComplaintAssignments", make_unique<MapType>(
        make_unique<TypeConst>("string"), make_unique<TypeConst>("string"))));
    globals.push_back(make_unique<Decl>("ComplaintRejections", make_unique<MapType>(
        make_unique<TypeConst>("string"), make_unique<TypeConst>("RejectionRecord"))));
    globals.push_back(make_unique<Decl>("ComplaintReassignmentCount", make_unique<MapType>(
        make_unique<TypeConst>("string"), make_unique<TypeConst>("number"))));
    
    // Plumber availability and specializations
    ...
    globals.push_back(make_unique<Decl>("PlumberAvailability", make_unique<MapType>(
        make_unique<TypeConst>("string"), make_unique<TypeConst>("boolean"))));
    ...
    inits.push_back(make_unique<Init>("ComplaintAssignments", 
        make_unique<Map>(vector<pair<unique_ptr<Var>,unique_ptr<Expr>>>())));
    inits.push_back(make_unique<Init>("ComplaintRejections", 
        make_unique<Map>(vector<pair<unique_ptr<Var>,unique_ptr<Expr>>>())));
    inits.push_back(make_unique<Init>("ComplaintReassignmentCount", 
        make_unique<Map>(vector<pair<unique_ptr<Var>,unique_ptr<Expr>>>())));
