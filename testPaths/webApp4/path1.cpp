// Path 1:
// Signup(Customer) → Login → Logout (Valid path, should return SAT)
...
static Program buildClientProgram()
{
    ...
    // logout_customer(customer_token);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("customer_token"));
        stmts.push_back(make_unique<FuncCallStmt>(
            make_unique<FuncCall>("logout_customer", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- logout_customer ---
    {
        // Precondition: customer_token exists in CustomerSessions
        vector<unique_ptr<Expr>> preArgs;
        ...
        auto callFn = make_unique<FuncCall>("logout_customer", move(args));

        // Postcondition: customer_token not in domain of CustomerSessions
        vector<unique_ptr<Expr>> postArgs;
        postArgs.push_back(make_unique<Var>("customer_token"));
        {
            vector<unique_ptr<Expr>> h;
            h.push_back(make_unique<Var>("CustomerSessions"));
