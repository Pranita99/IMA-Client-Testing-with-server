...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for register_student → authenticate → saveRequest → accept → getBookStudentsByStudentId
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    decls.push_back(make_unique<Decl>("book_id",
                     make_unique<TypeConst>("string")));
    // book_id = input(); // Book ID to assign to student
    {
    ...
    // accept(admin_token, request_id, book_id);
    {
        vector<unique_ptr<Expr>> a;
    ...
    // getBookStudentsByStudentId(token, student_id);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("token"));
        a.push_back(make_unique<Var>("student_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("getBookStudentsByStudentId", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- Save Request API block ---
    {
        ...
        /* post: R[request_id] = {type: type, description: description, priority: priority, student_id: T[token], status: "pending"} */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> idx;
        ...
        }
        {
            ...
            record.push_back(make_unique<Var>("pending"));
    ...
    // --- Accept API block ---
    {
        /* pre: admin_token ∈ dom(A) && request_id ∈ dom(R) && R[request_id].status = "pending" && book_id ∈ dom(B) */
        vector<unique_ptr<Expr>> land;
        {
            vector<unique_ptr<Expr>> inArgs;
        ...
        /* post: R[request_id].status = "accepted" && BS[book_id].student_id = R[request_id].student_id */
        vector<unique_ptr<Expr>> land_post;
        {
            vector<unique_ptr<Expr>> eq1;
            {
                ...
                statusArgs.push_back(make_unique<Var>("R"));
                statusArgs.push_back(make_unique<Var>("request_id"));
                eq1.push_back(make_unique<FuncCall>("get_status", move(statusArgs)));
            }
            eq1.push_back(make_unique<Var>("accepted"));
            land_post.push_back(make_unique<FuncCall>("equals", move(eq1)));
        }
        {
            vector<unique_ptr<Expr>> eq2;
            {
                vector<unique_ptr<Expr>> bsArgs;
                bsArgs.push_back(make_unique<Var>("BS"));
                bsArgs.push_back(make_unique<Var>("book_id"));
                eq2.push_back(make_unique<FuncCall>("get_student_id", move(bsArgs)));
            }
            {
                vector<unique_ptr<Expr>> reqArgs;
                reqArgs.push_back(make_unique<Var>("R"));
                reqArgs.push_back(make_unique<Var>("request_id"));
                eq2.push_back(make_unique<FuncCall>("get_student_id", move(reqArgs)));
            }
            land_post.push_back(make_unique<FuncCall>("equals", move(eq2)));
        }
    ...
    // --- Get Book Students By Student ID API block ---
    {
        /* pre: token ∈ dom(T) && T[token] ∈ dom(S) && student_id ∈ dom(S) */
        vector<unique_ptr<Expr>> land;
        {
            vector<unique_ptr<Expr>> inArgs;
            inArgs.push_back(make_unique<Var>("token"));
        ...
        callArgs.push_back(make_unique<Var>("token"));
        callArgs.push_back(make_unique<Var>("student_id"));
        auto callFn = make_unique<FuncCall>("getBookStudentsByStudentId", move(callArgs));

        /* post: returns all books assigned to student_id from BS */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> filterArgs;
            filterArgs.push_back(make_unique<Var>("BS"));
        ...
        postArgs.push_back(make_unique<Var>("book_assignments"));
        auto post = make_unique<FuncCall>("equals", move(postArgs));
    ...
    globals.push_back(make_unique<Decl>(
        "T", make_unique<MapType>(
                 make_unique<TypeConst>("string"),
                 make_unique<TypeConst>("string"))));
    // Requests map: R[request_id] = {type, description, priority, student_id, status}
    globals.push_back(make_unique<Decl>(
    ...
    // Book-Student assignments: BS[book_id] = {student_id, assignment_date}
    globals.push_back(make_unique<Decl>(
