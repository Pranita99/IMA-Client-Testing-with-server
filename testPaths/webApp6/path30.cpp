...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for register_student → authenticate → getAllBooks → saveRequest → accept → getBookStudentById
// ─────────────────────────────────────────────────────────────
static Program buildStudentClientProgram()
{
    ...
    // saveRequest(request_id, book_id);
    {
        vector<unique_ptr<Expr>> a;
    ...
    // request_id = input();   (for accept operation)
    {
        auto lhs = make_unique<Var>("request_id");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
    ...
    decls.push_back(make_unique<Decl>("student_id",
                     make_unique<TypeConst>("string")));
    // student_id = input();   (for getBookStudentById operation)
    {
    ...
    // getBookStudentById(student_id);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("student_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("getBookStudentById", move(a))));
    }
...
// ─────────────────────────────────────────────────────────────
//  Build the API *Spec* AST with student book request management functionality
// ─────────────────────────────────────────────────────────────
static Spec buildStudentSpec()
{
    ...
    // --- Get All Books API block ---
    {
        ...
        /* post: returns all books B */
        vector<unique_ptr<Expr>> postArgs;
        postArgs.push_back(make_unique<Var>("all_books"));
    ...
    // --- Accept Request API block ---
    {
        /* pre: request_id ∈ dom(R) */
        vector<unique_ptr<Expr>> inDom;
        ...
        /* post: BS[R[request_id]] = ST[student_token] */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> idx;
            idx.push_back(make_unique<Var>("BS"));
            {
                vector<unique_ptr<Expr>> innerIdx;
                innerIdx.push_back(make_unique<Var>("R"));
                innerIdx.push_back(make_unique<Var>("request_id"));
                idx.push_back(make_unique<FuncCall>("mapped_value", move(innerIdx)));
            }
            postArgs.push_back(make_unique<FuncCall>("mapped_value", move(idx)));
        }
        {
    ...
    // --- Get Book Student By ID API block ---
    {
        ...
        vector<unique_ptr<Expr>> inDom;
        inDom.push_back(make_unique<Var>("S"));
        inDom.push_back(make_unique<Var>("student_id"));
        auto pre = make_unique<FuncCall>("in_dom", move(inDom));
        ...
        /* post: returns student's book assignments where student_id is in BS */
        vector<unique_ptr<Expr>> postArgs;
        postArgs.push_back(make_unique<Var>("student_books"));
        {
            vector<unique_ptr<Expr>> filterArgs;
            filterArgs.push_back(make_unique<Var>("BS"));
            filterArgs.push_back(make_unique<Var>("student_id"));
            postArgs.push_back(make_unique<FuncCall>("filter_by_value", move(filterArgs)));
        }
        auto post = make_unique<FuncCall>("equals", move(postArgs));
    ...
    globals.push_back(make_unique<Decl>(
        "R", make_unique<MapType>(
                 make_unique<TypeConst>("string"),
                 make_unique<TypeConst>("string"))));
    // Book-Students map: BS[book_id] = student_username
    globals.push_back(make_unique<Decl>(
...
Program clientProgram = buildStudentClientProgram();
Spec    spec          = buildStudentSpec();