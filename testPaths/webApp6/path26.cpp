...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for authenticate → saveBook → deleteBook → getBook(fail)
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // book_id = input();   (for delete operation, reusing same book_id variable)
    {
        auto lhs = make_unique<Var>("book_id");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
    ...
    // book_id = input();   (for get operation - this should fail since book was deleted)
    {
        auto lhs = make_unique<Var>("book_id");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
    ...
    // getBook(book_id);  (this will fail since the book was deleted)
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("book_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
...
// ─────────────────────────────────────────────────────────────
//  Build the API *Spec* AST with admin book management functionality including delete
// ─────────────────────────────────────────────────────────────
static Spec buildSpec()
{
    ...
    // --- Delete Book API block ---
    {
        ...
        postArgs.push_back(make_unique<Var>("book_id"));
        {
            ...
            postArgs.push_back(make_unique<FuncCall>("dom", move(h)));
        }
