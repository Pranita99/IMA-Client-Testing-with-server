...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for authenticate(fail: not_registered) → authenticate(success)
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // authenticate(username, password); // First attempt - will fail (not registered)
    {
    ...
    decls.push_back(make_unique<Decl>("registered_username",
                     make_unique<TypeConst>("string")));
    // registered_username = input();
    {
        auto lhs = make_unique<Var>("registered_username");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(args))));
    ...
    decls.push_back(make_unique<Decl>("registered_password",
                     make_unique<TypeConst>("string")));
    // registered_password = input();
    {
        auto lhs = make_unique<Var>("registered_password");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(args))));
    ...
    // authenticate(registered_username, registered_password); // Second attempt - will succeed
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("registered_username"));
        a.push_back(make_unique<Var>("registered_password"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("authenticate", move(a))));
    }
...
// ─────────────────────────────────────────────────────────────
//  Build the API *Spec* AST with authentication failure and success scenarios
// ─────────────────────────────────────────────────────────────
static Spec buildSpec()
{
    vector<unique_ptr<API>> apiBlocks;

    // --- Authenticate API block (Failure case - user not registered) ---
    {
        /* pre: not_in(u, dom(U)) */
        ...
        /* post: T remains unchanged (no token created) */
        vector<unique_ptr<Expr>> postArgs;
        postArgs.push_back(make_unique<Var>("T"));
        postArgs.push_back(make_unique<Var>("T"));
    ...
    // --- Authenticate API block (Success case - valid credentials) ---
    {
    ...
    // User credentials map: U[username] = password
