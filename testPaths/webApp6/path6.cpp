...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for register_student → authenticate → getStudent → updateStudent → getStudent
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // getStudent(token); // First call to get student info
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("token"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("getStudent", move(a))));
    }

    // string new_email;
    decls.push_back(make_unique<Decl>("new_email",
                     make_unique<TypeConst>("string")));
    // new_email = input();
    {
        auto lhs = make_unique<Var>("new_email");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
        stmts.push_back(make_unique<Assign>(move(lhs),
                         make_unique<FuncCall>("input", move(args))));
    ...
    // updateStudent(token, new_email);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("token"));
        a.push_back(make_unique<Var>("new_email"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("updateStudent", move(a))));
    }

    // getStudent(token); // Second call to verify updated student info
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("token"));
        stmts.push_back(make_unique<FuncCallStmt>(
...
// ─────────────────────────────────────────────────────────────
//  Build the API *Spec* AST with student management operations
// ─────────────────────────────────────────────────────────────
static Spec buildSpec()
{
    ...
    // --- Register Student API block ---
    {
        ...
        callArgs.push_back(make_unique<Var>("e"));
        auto callFn = make_unique<FuncCall>("register_student", move(callArgs));

        /* post: S[u] = {password: p, email: e} */
        ...
        }
        {
            ...
            record.push_back(make_unique<Var>("p"));
            record.push_back(make_unique<Var>("e"));
    ...
    // --- Authenticate API block (Success case) ---
    {
    ...
    // --- Get Student API block ---
    {
        ...
        auto callFn = make_unique<FuncCall>("getStudent", move(callArgs));

        /* post: returns S[T[token]] */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> studentArgs;
            studentArgs.push_back(make_unique<Var>("S"));
            {
                vector<unique_ptr<Expr>> tokenArgs;
                tokenArgs.push_back(make_unique<Var>("T"));
                tokenArgs.push_back(make_unique<Var>("token"));
                studentArgs.push_back(make_unique<FuncCall>("mapped_value", move(tokenArgs)));
            }
            postArgs.push_back(make_unique<FuncCall>("mapped_value", move(studentArgs)));
        }
    ...
    // --- Update Student API block ---
    {
        ...
        vector<unique_ptr<Expr>> callArgs;
        callArgs.push_back(make_unique<Var>("token"));
        callArgs.push_back(make_unique<Var>("new_email"));
        auto callFn = make_unique<FuncCall>("updateStudent", move(callArgs));

        /* post: S[T[token]].email = new_email */
        vector<unique_ptr<Expr>> postArgs;
        {
            vector<unique_ptr<Expr>> emailArgs;
            emailArgs.push_back(make_unique<Var>("S"));
            {
                vector<unique_ptr<Expr>> tokenArgs;
                tokenArgs.push_back(make_unique<Var>("T"));
                tokenArgs.push_back(make_unique<Var>("token"));
                emailArgs.push_back(make_unique<FuncCall>("mapped_value", move(tokenArgs)));
            }
            postArgs.push_back(make_unique<FuncCall>("get_email", move(emailArgs)));
        }
        postArgs.push_back(make_unique<Var>("new_email"));
        auto post = make_unique<FuncCall>("equals", move(postArgs));
