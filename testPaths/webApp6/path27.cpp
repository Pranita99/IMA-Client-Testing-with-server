...
// ─────────────────────────────────────────────────────────────
//  Build the *client* Program AST for register_student → authenticate → getAllBooks → saveRequest → accept → getBookStudentsByBookId
// ─────────────────────────────────────────────────────────────
static Program buildClientProgram()
{
    ...
    // book_id = input();   (for getBookStudentsByBookId operation)
    {
        auto lhs = make_unique<Var>("book_id");
        vector<unique_ptr<Expr>> args;
        args.push_back(make_unique<Var>(""));
    ...
    // getBookStudentsByBookId(book_id);
    {
        vector<unique_ptr<Expr>> a;
        a.push_back(make_unique<Var>("book_id"));
        stmts.push_back(make_unique<FuncCallStmt>(
              make_unique<FuncCall>("getBookStudentsByBookId", move(a))));
    }
...
static Spec buildSpec()
{
    ...
    // --- Get Book Students By Book ID API block ---
    {
        ...
        /* post: returns students who have this book BS[book_id] */
        vector<unique_ptr<Expr>> postArgs;
        postArgs.push_back(make_unique<Var>("book_students"));
        {
            vector<unique_ptr<Expr>> idx;
            idx.push_back(make_unique<Var>("BS"));
